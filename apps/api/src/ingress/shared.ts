...
/**
 * Shared plumbing for the data-ingress handlers. Every inbound data source
 * (the desktop agent heartbeat, the Genesys relay, the Clockodo webhook)
 * flows through a handler with the same shape:
 *   (IngressRequest) => Promise<IngressResult>
 * Auth, response shaping, and the secret-guarded push to Convex live here
 * once, rather than being copy-pasted per route.
 */
...
/** What a handler returns; index.ts applies it to the Elysia response. */
...
/**
 * Validate a desktop agent's device token. Returns `{ deviceId }` when the
 * token is good and its device is active, else `null`.
 */
