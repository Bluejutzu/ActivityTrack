...
; written to update-task-register.log next to config.json -- the identical
; command ran clean by hand on a real machine but silently failed to
; register the task when run through nsExec, so the *NSIS invocation itself*
; needs visibility into what's actually happening (e.g. NSIS_MAX_STRLEN
; truncating the ~1580-character command line) rather than another guess.
;
...
!macro NSIS_HOOK_POSTINSTALL
  ...
  Pop $2
  Pop $3
  FileOpen $1 "$COMMONAPPDATA\ActivityTrack\update-task-register.log" w
