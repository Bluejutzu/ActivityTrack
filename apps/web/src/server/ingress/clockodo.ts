...
/**
 * POST /api/webhooks/clockodo — Clockodo time-tracking webhook. Handles the
 * shapes Clockodo actually sends, plus two legacy adapter shapes:
 *
 *   A. Validation handshake — `{ secret }` on create / URL change. We log it
 *      (paste it into Clockodo's "Token" field) and 200 so validation passes.
